...
func main() {
	...
	switch choice {
	case 1:
		// Clear the Files, like links -- only
		// Also change the file names to be simple, like MDL_Links.csv
	...
	case 2:
		...
		scraper := extract.NewTenderDataScraper(sess, domain, state, runDate)
...
func getRunDate() string {
	...
	// Sort folders by modification time descending
	sort.Slice(folders, func(i, j int) bool {
		infoI, errI := folders[i].Info()
		infoJ, errJ := folders[j].Info()
